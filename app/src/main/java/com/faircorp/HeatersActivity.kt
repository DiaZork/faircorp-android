...
import com.faircorp.adapter.HeatersAdapter
...
class HeatersActivity : BasicActivity(), OnHeaterSelectedListener {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val adapter = HeatersAdapter(this) // (3)
        ...
        lifecycleScope.launch(context = Dispatchers.IO) { // (1)
            runCatching { MyApiServices().heaterApiService.findAll().execute() } // (2)
                .onSuccess {
    ...
    override fun onHeaterSelected(id: Long) {
        val intent = Intent(this, HeaterActivity::class.java).putExtra(HEATER_ID, id)
